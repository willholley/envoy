...
router.get('/:db/_all_docs', auth.isAuthenticated, function(req, res) {
  ...
  if (req.query.keys) {
    ...
    var removeDocs = (typeof req.query.include_docs === 'undefined' || req.query.include_docs === 'false');
    req.query.include_docs = true;

    req.query.keys = JSON.parse(req.query.keys);
    console.log(req.query, removeDocs);    
    // hit the real all_docs
    app.db.list(req.query)
      .pipe(utils.liner())
      .pipe(utils.authRemover(req.session.user.name, removeDocs))
      .pipe(res);
